...
interface MyBookAudioBookInfoProps {
  data: IBookSlug | undefined;
}

const MyBookAudioBookInfo: FC<MyBookAudioBookInfoProps> = ({ data }) => {
  return (
    <div className="flex flex-col gap-y-[30px]">
      <h1 className="first-letter:uppercase leading-[130%] text-5xl font-semibold">
        {data?.title}
      </h1>
      <h4 className="leading-[130%] text-lg">{data?.author[0].name}</h4>
      <p className="leading-[150%] text-sm">{data?.description}</p>
      ...
      <div className="font-semibold leading-[130%] text-lg">
