...
import { BkCommentAudio, BkNextAudio, BkPlayAudio, BkPrevAudio } from "@/assets/images";
    ...
    <div className='bg-[#d7e7f8] py-[30px] rounded-[16px] px-[40px] flex items-center justify-center w-full h-[166px] max-[820px]:p-[20px] max-[600px]:p-[10px'>
      <div className='flex items-center flex-col w-[560px]'>
        <div className='flex items-center justify-center gap-[36px] w-full'>
          <img className="cursor-pointer" onClick={clickBookInfo} src={BkCommentAudio} alt="prev" />
          <img className="cursor-pointer" onClick={prevAudio} src={BkPrevAudio} alt="prev" />
          {isPlaying ? (
            <HiPause className="cursor-pointer text-[#2d71ae] text-[57.5px]" onClick={pauseAudio} />
          ) : (
            <img className="cursor-pointer" onClick={playAudio} src={BkPlayAudio} alt="play" />
          )}
          <img className="cursor-pointer" onClick={nextAudio} src={BkNextAudio} alt="next" />
        </div>
        <div className='flex flex-col w-full gap-y-[10px]'>
          <div className='w-full flex items-center justify-between'>
            <div className="text-[#2d71ae]">{formatTime(time)}</div>
            <div className="text-[#2d71ae]">{formatTime(audio?.current?.duration || 0)}</div>
          </div>
          <div>
            <Slider
              tooltip={{ open: false }}
